...
def run():
    ...
    for _ in range(5000000):
        ...
        if len(processed_junctions) == len(junction_boxes):
            print("All junction boxes are now connected")
            print("Final distance pair processed:")
            print(f"{junction_boxes[idx1]} <-> {junction_boxes[idx2]}")
            print(f"Mulitplied x-coordinates: {junction_boxes[idx1][0] * junction_boxes[idx2][0]}")
            break
